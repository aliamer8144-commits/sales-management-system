  ...
  try {
    ...
    await db.execute({
      sql: `INSERT INTO PurchaseInvoice (id, userId, totalAmount, notes, createdAt, updatedAt)
            VALUES (?, ?, ?, ?, ?, ?)`,
      args: [invoiceId, userId, totalAmount, notes || null, getCurrentDate(), getCurrentDate()],
    });
    ...
    for (const item of invoiceItems) {
      await db.execute({
        sql: `INSERT INTO PurchaseInvoiceItem (id, purchaseInvoiceId, productId, productUnitId, quantity, purchasePrice, unitType, unitName, createdAt)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [item.id, item.purchaseInvoiceId, item.productId, item.productUnitId, item.quantity, item.purchasePrice, item.unitType, item.unitName, getCurrentDate()],
      });
    }
    ...
    for (const [productId, productItems] of itemsByProduct) {
      ...
      for (const item of productItems) {
        if (item.purchasePrice && item.purchasePrice > 0) {
          ...
          if (unit) {
            await db.execute({
              sql: `UPDATE ProductUnit SET purchasePrice = ?, updatedAt = ? WHERE id = ?`,
              args: [item.purchasePrice, getCurrentDate(), item.productUnitId],
            });
          }
        }
      }
