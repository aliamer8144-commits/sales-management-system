          ...
          {selectedItems.length > 0 && (
            <div className="border-t border-slate-300 bg-slate-100">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-bold text-teal-600">{formatCurrency(total)}</span>
              </div>
            </div>
          )}
        </div>
                ...
                <div className="flex items-center justify-between p-3 bg-amber-50 rounded-lg border border-amber-200">
                  <div>
                    ...
                    {selectedCustomer.phone && (
                      <p className="text-sm text-amber-600">{selectedCustomer.phone}</p>
                    )}
                  </div>
                  <button
                ...
                <div className="space-y-2">
                  {customers.slice(0, 5).map((customer) => (
                    <button
                      ...
                      className="w-full p-3 text-right bg-gray-50 hover:bg-amber-50 rounded-lg border border-gray-200 hover:border-amber-300 transition-colors"
                    >
                  ...
                  {customers.length === 0 && (
                    <p className="text-sm text-gray-500 text-center py-2">لا يوجد عملاء</p>
                  )}
                </div>
              )}
            </div>
...
function ProductSearchRow({
  ...
  // Get available units with actual stock
  const availableUnits = product.units.filter(unit => unit.stockQuantity > 0);
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                <Input
                  ...
                  min="1"
                  max={selectedUnit.stockQuantity}
                  autoFocus
                />
              </div>
              <span className="text-xs text-slate-400 font-medium">/ {selectedUnit.stockQuantity}</span>
            </div>
