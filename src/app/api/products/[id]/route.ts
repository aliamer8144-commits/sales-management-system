...
import { db, getCurrentDate } from '@/lib/db';
...
export async function PUT(
  ...
  try {
    ...
    if (hasAlertUnitTypeColumn) {
      await db.execute({
        sql: `UPDATE Product SET name = ?, alertLimit = ?, alertUnitType = ?, notes = ?, updatedAt = ? WHERE id = ?`,
        args: [name, alertLimit || 5, alertUnitType || 'piece', notes || null, getCurrentDate(), id],
      });
    } else {
      await db.execute({
        sql: `UPDATE Product SET name = ?, alertLimit = ?, notes = ?, updatedAt = ? WHERE id = ?`,
        args: [name, alertLimit || 5, notes || null, getCurrentDate(), id],
      });
    }
